/**
 * =>herois sao cadstrados no mongodb com nome e poder 
 * 
 * =>usuarios sao cadastrados no postgres
 * com nome e senha
 * 
 * 
 */
...
describe('==> Auth User Suite de Testes', function () {
    this.beforeAll(async () => {
        ...
        //cadatro do mock user_db pois nao tem rota de cadastro de usuario
        /**
        *
        para isso mudar a assinatura do metodo update do postgres pois esta como unico para:
        * upset = insere
        pois assim se ele nao encontra vai inserir no banco nao vai ser necessario fica deletando para novo teste
        ...
        *
        * 
        altera tambem update do contextStrategy.js para:
...
/**
 * apos isso ir na pasta schemas do postgres fazer userSchema.js
 */
    ...
    login() {
        ...
        return {
            ...
            handler: async (request, headers) => {
                ...
                //compara a senha que veio e valida com hash no banco de dados
...
/**
 * apos importa o banco e rotas na api.js
 */
const {
    join
} = require('path')
...
async function main() {
    ...
    //conexao para postgres e schema userSchema
    const connectionPostgres = await PostgresDB.connect()
    const model = await PostgresDB.defineModel(connectionPostgres, UserSchema)
    const postgresModel = new Context(new PostgresDB(connectionPostgres, model));
    ...
    app.route([
        ...
        //adicona conexao e schama para user atraves do postgresModel
